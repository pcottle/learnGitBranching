exports.level = {
  ...
  "hint": {
    ...
    "bg": "Просто напишете 'git commit' два пъти, за да приключите ;)",
    "ru_RU": "Попробуй дважды выполнить команду 'git commit' ;)",
    "uk": "Спробуйте двічі виконати команду 'git commit' ;)",
    ...
    "zh_CN": {
      "childViews": [
        ...
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "咱们来实际操作一下，看看提交记录是怎样的。右边展示了一个（小型）Git 代码库的可视化图。当前有两个提交记录 —— 初始提交 `C0` 和其后可能包含某些有用修改的提交 `C1`。",
              "",
              "点击下面的按钮创建一个新的提交记录。"
            ],
            "afterMarkdowns": [
              "好了！非常棒！我们刚才修改了代码库，并把这些修改保存成了一个提交记录 `C2`。`C2` 的 parent 节点是 `C1`， 它指明了这次提交是基于哪个提交产生的。"
            ],
            "command": "git commit",
            "beforeCommand": ""
          }
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## Git Комити",
              "",
              "Един комит в Git хранилище записва моментна снимка (snapshot) на всички (проследявани) файлове във вашата директория. Това е като огромно копиране и поставяне, но дори по-добро!",
              "",
              "Git се стреми да поддържа комитите възможно най-леки, така че не копира сляпо цялата директория всеки път, когато правите комит. Той може (когато е възможно) да компресира комит като набор от промени или \"делта\" от една версия на хранилището към следващата.",
              "",
              "Git също така поддържа история за това кои комити кога са направени. Ето защо повечето комити имат предци над себе си -- ние обозначаваме това със стрелки в нашата визуализация. Поддържането на история е чудесно за всеки, който работи по проекта!",
              "",
              "Информацията е много, но засега можете да мислите за комитите като за моментни снимки на проекта. Комитите са много леки и превключването между тях е адски бързо!"
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Нека видим как изглежда това на практика. Отдясно имаме визуализация на (малко) Git хранилище. В момента има два комита -- първият начален комит, `C0`, и един комит след него, `C1`, който може да съдържа някои значими промени.",
              "",
              "Натиснете бутона по-долу, за да направите нов комит."
            ],
            "afterMarkdowns": [
              "Готово! Страхотно. Току-що направихме промени в хранилището и ги запазихме като комит. Комитът, който току-що направихме, има родител, `C1`, който показва на базата на кой комит е създаден."
            ],
            "command": "git commit",
            "beforeCommand": ""
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "Давайте, опитайте сами! След като този прозорец се затвори, направете два комита, за да завършите нивото."
            ]
          }
        }
      ]
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "Тут є багато над чим подумати, але наразі Ви можете уявляти коміти як моментальні знімки проекту. Коміти майже невагомі й перемикання між ними дуже швидке."
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Подивімось, як це виглядає на практиці. Справа зображена візуалізація маленького Git-репозиторію. Наразі ми бачимо два коміти: початковий коміт `C0`, та наступний коміт `C1`, який містить якісь змістовні зміни.",
              "",
              "Натисніть кнопку нижче, щоб створити новий коміт."
            ],
            "afterMarkdowns": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "Спробуйте самі. Після того як це вікно закриється, зробіть два коміти, щоб пройти цей рівень."
            ]
          }
        }
      ]
