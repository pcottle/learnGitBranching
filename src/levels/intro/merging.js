      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## Клонове и сливане",
              "",
              "Чудесно! Вече знаем как да правим комити и клонове. Сега трябва да научим начин за комбиниране на работата от два различни клона. Това ще ни позволи да създадем нов клон, да разработим нова функционалност и след това да я вкараме обратно.",
              "",
              "Първият метод за комбиниране на работа, който ще разгледаме, е `git merge`. Сливането в Git създава специален комит, който има двама уникални родители. Комит с двама родители по същество означава: \"Искам да включа цялата работа от този родител тук и от онзи там, *и* набора от всички техни родители.\"",
              "",
              "По-лесно е с визуализации, нека го разгледаме в следващия екран."
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Тук имаме два клона; всеки има по един уникален комит. Това означава, че нито един от клоновете не включва пълния набор от \"работа\" в хранилището, която сме свършили. Нека поправим това със сливане (merge).",
              "",
              "Ще слеем (`merge`) клона `bugFix` в `main`."
            ],
            "afterMarkdowns": [
              "Уха! Видяхте ли това? Първо, `main` сега сочи към комит, който има двама родители. Ако проследите стрелките нагоре по дървото с комити от `main`, ще преминете през всеки комит по пътя до корена. Това означава, че `main` сега съдържа цялата работа в хранилището.",
              "",
              "Също така, виждате ли как цветовете на комитите се промениха? За да помогна с ученето, включих цветова координация. Всеки клон има уникален цвят. Всеки комит приема цвят, който е смесена комбинация от всички клонове, които съдържат този комит.",
              "",
              "Така че тук виждаме, че цветът на клона `main` е смесен във всички комити, но цветът на `bugFix` не е. Нека поправим това..."
            ],
            "command": "git merge bugFix",
            "beforeCommand": "git checkout -b bugFix; git commit; git checkout main; git commit"
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Нека слеем `main` в `bugFix`:"
            ],
            "afterMarkdowns": [
              "Тъй като `bugFix` беше предшественик на `main`, git не трябваше да върши никаква работа; просто премести `bugFix` към същия комит, към който беше прикачен `main`.",
              "",
              "Сега всички комити са с един и същи цвят, което означава, че всеки клон съдържа цялата работа в хранилището! Юху!"
            ],
            "command": "git checkout bugFix; git merge main",
            "beforeCommand": "git checkout -b bugFix; git commit; git checkout main; git commit; git merge bugFix"
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "За да завършите това ниво, изпълнете следните стъпки:",
              "",
              " * Направете нов клон, наречен `bugFix`",
              " * Превключете към клона `bugFix` с `git checkout bugFix`",
              " * Направете един комит",
              " * Върнете се към `main` с `git checkout`",
              " * Направете още един комит",
              " * Слейте клона `bugFix` в `main` с `git merge`",
              "",
              " *Запомнете, винаги можете да покажете този диалог отново с \"objective\"!*"
            ]
          }
        }
      ]
        ...
        },
        {
          ...
          "options": {
            ...
            "afterMarkdowns": [
              ...
              "",
              "Во-вторых, обрати внимание, как изменились цвета коммитов. Мы ввели цветовую дифференциацию, чтобы помочь пониманию. У каждой ветки — свой цвет. Каждый коммит принимает цвет, представляющий собой смесь цветов всех веток, которые содержат этот коммит.",
              "",
