      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## Git Тагове",
              "",
              "Както научихте от предишните уроци, клоновете се местят лесно и често препращат към различни комити, докато работата по тях приключи. Клоновете се променят лесно, често са временни и винаги се менят.",
              "",
              "В такъв случай може би се чудите дали има начин *постоянно* да маркирате исторически точки в историята на вашия проект. За неща като големи релийзи (releases) и големи сливания (merges), има ли начин да маркираме тези комити с нещо по-постоянно от клон?",
              ""
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "Разбира се, че има! Git таговете (етикетите) поддържат точно този случай на употреба -- те (донякъде) постоянно маркират определени комити като \"ключови етапи\" (milestones), към които можете да реферирате като към клон.",
              "",
              "",
              "",
              "По-важното обаче е, че те никога не се местят, когато се създават нови комити. Не можете да направите \"check out\" на таг и след това да завършите работа по него -- таговете съществуват като котви (anchors) в дървото с комити, които обозначават определени места.",
              "",
              "Нека видим как изглеждат таговете на практика."
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Нека се опитаме да направим таг на `C1`, който е нашият прототип на версия 1."
            ],
            "afterMarkdowns": [
              "Ето! Доста лесно. Именувахме тага `v1` и изрично посочихме комита `C1`. Ако пропуснете комита, git просто ще използва това, където се намира `HEAD`."
            ],
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "За това ниво просто създайте таговете от визуализацията на целта и след това направете checkout на `v1`. Забележете как влизате в състояние на отделен (detached) `HEAD` -- това е така, защото не можете да комитвате директно върху тага `v1`.",
              "",
              "",
              "",
              "В следващото ниво ще разгледаме по-интересен случай на употреба на тагове."
            ]
          }
        }
      ]
