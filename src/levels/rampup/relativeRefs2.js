      ...
      "childViews": [
        ...
        },
        {
          ...
          "options": {
            ...
            "afterMarkdowns": [
              "就是这样! 相对引用让我们能简洁地指向 `C1`，而强制修改分支（`-f`）则让我们能快速地将一个分支移动到那个位置。"
            ],
            "command": "git branch -f main HEAD~3",
            "beforeCommand": "git commit; git commit; git commit; git checkout -b bugFix"
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "既然你已经见识了相对引用和强制修改分支的结合使用，那么让我们使用它们来完成这一关吧！",
              "",
  ...
  "childViews": [
    {
      ...
      "options": {
        "markdowns": [
          "### Операторът \"~\"",
          "",
          "Да кажем, че искаш да се върнеш много нива нагоре в дървото от комити. Би било досадно да пишеш `^` много пъти, затова Git има и оператора тилда (~).",
          "",
          "",
          "Операторът тилда (по избор) приема число, което указва колко родителски комита искаш да изкачиш. Нека го видим в действие."
        ]
      }
    },
    {
      ...
      "options": {
        "beforeMarkdowns": [
          "Нека укажем броя комити назад с `~`."
        ],
        "afterMarkdowns": [
          "Бум! Толкова кратко и ясно — относителните референции са страхотни."
        ],
        "command": "git checkout HEAD~4",
        "beforeCommand": "git commit; git commit; git commit"
      }
    },
    {
      ...
      "options": {
        "markdowns": [
          "### Принудително преместване на клонове",
          "",
          "Вече си експерт по относителните референции, така че нека ги *използваме* за нещо практично.",
          "",
          "Един от най-честите начини, по които използвам относителни референции, е за преместване на клонове. Можеш директно да пренасочиш клон към даден комит с опцията `-f`. Например:",
          "",
          "`git branch -f main HEAD~3`",
          "",
          "премества (принудително) клона main три родителя назад спрямо HEAD.",
          "",
          "*Забележка: В реална git среда командата `git branch -f` не е позволена за текущия клон.*"
        ]
      }
    },
    {
      ...
      "options": {
        "beforeMarkdowns": [
          "Нека видим тази команда в действие."
        ],
        "afterMarkdowns": [
          "Готово! Относителните референции ни дадоха кратък начин да посочим `C1`, а принудителното преместване (`-f`) — бърз начин да преместим клона там."
        ],
        "command": "git branch -f main HEAD~3",
        "beforeCommand": "git commit; git commit; git commit; git checkout -b bugFix"
      }
    },
    {
      ...
      "options": {
        "markdowns": [
          "След като видя относителните референции и принудителното преместване на клонове заедно, нека ги използваме, за да решим следващото ниво.",
          "",
          "За да завършиш това ниво, премести `HEAD`, `main` и `bugFix` до показаните целеви позиции."
        ]
      }
    }
  ]
