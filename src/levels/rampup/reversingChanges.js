      ...
      "childViews": [
        ...
        },
        {
          ...
          "options": {
            ...
            "command": "git reset HEAD~1",
            "afterMarkdowns": [
              "漂亮! Git 把 main 分支移回到了 `C1`；现在我们的本地仓库处于一种仿佛 `C2` 从未发生过的状态。",
              "",
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "要完成此关，请分别撤销 `local` 分支和 `pushed` 分支上的最近一次提交。你总共需要撤销两个提交（每个分支一个）。",
              "",
              "请记住，`pushed` 是远程分支，`local` 是本地分支 —— 这应该能帮助你选择合适的方法。"
            ]
          }
        }
      ]
    ...
    "ru_RU": {
      "childViews": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "Чтобы пройти этот уровень, отмени самый последний коммит на ветках `local` и `pushed`.",
              "Всего будет отменено два коммита (по одному на ветку)",
              "",
