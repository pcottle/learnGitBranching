exports.level = {
  ...
  "hint": {
    ...
    "bg": "Просто изпълни git fetch!",
    "ru_RU": "Просто выполните git fetch!",
    "uk": "Просто виконайте git fetch!",
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## Git Fetch",
              "",
              "کار با ریموت‌های گیت واقعاً فقط به انتقال داده‌ها _به_ و _از_ سایر مخازن خلاصه می‌شود. تا زمانی که بتوانیم کامیت‌ها را رد و بدل کنیم، می‌توانیم هر نوع به‌روزرسانی که توسط گیت ردیابی می‌شود را به اشتراک بگذاریم (و بنابراین کار، فایل‌های جدید، ایده‌های جدید، نامه‌های عاشقانه و غیره را به اشتراک بگذاریم).",
              "",
              "در این درس یاد خواهیم گرفت که چگونه داده‌ها را _از_ یک مخزن ریموت دریافت کنیم -- دستور این کار به راحتی `git fetch` نامیده می‌شود.",
              "",
              "متوجه خواهید شد که با به‌روزرسانی بازتاب ما از مخزن ریموت، شاخه‌های _ریموت_ ما نیز به‌روز می‌شوند تا آن بازتاب جدید را منعکس کنند. این موضوع به درس قبلی در مورد شاخه‌های ریموت مرتبط است."
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "قبل از وارد شدن به جزئیات `git fetch`، بیایید آن را در عمل ببینیم! در اینجا ما یک مخزن ریموت داریم که حاوی دو کامیت است که مخزن محلی ما ندارد."
            ],
            "afterMarkdowns": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "### دستور fetch چه می‌کند",
              "",
              "`git fetch` دو مرحله اصلی، و تنها دو مرحله اصلی را انجام می‌دهد. این دستور:",
              "",
              "* کامیت‌هایی را که ریموت دارد اما در مخزن محلی ما وجود ندارند دانلود می‌کند، و...",
              "* جایی که شاخه‌های ریموت ما اشاره می‌کنند (مثلاً `o/main`) را به‌روزرسانی می‌کند.",
              "",
              "`git fetch` اساساً بازتاب _محلی_ ما از مخزن ریموت را با آنچه مخزن ریموت _واقعی_ (در حال حاضر) به آن شباهت دارد، همگام‌سازی می‌کند.",
              "",
              "اگر از درس قبلی به یاد داشته باشید، گفتیم که شاخه‌های ریموت وضعیت مخازن ریموت را _از_ آخرین باری که با آن ریموت‌ها صحبت کردید، منعکس می‌کنند. `git fetch` راهی است که شما با این ریموت‌ها صحبت می‌کنید! امیدوارم ارتباط بین شاخه‌های ریموت و `git fetch` اکنون آشکار باشد.",
              "",
              "`git fetch` معمولاً از طریق اینترنت (از طریق پروتکلی مانند `http://` یا `git://`) با مخزن ریموت صحبت می‌کند."
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "### دستور fetch چه کاری انجام نمی‌دهد",
              "",
              "با این حال، `git fetch` هیچ چیزی را در مورد وضعیت محلی _شما_ تغییر نمی‌دهد. این دستور شاخه `main` شما را به‌روز نمی‌کند و هیچ چیزی در مورد نحوه ظاهر شدن سیستم فایل شما در حال حاضر تغییر نمی‌دهد.",
              "",
              "درک این موضوع مهم است زیرا بسیاری از توسعه‌دهندگان فکر می‌کنند که اجرای `git fetch` باعث می‌شود کار محلی آن‌ها وضعیت ریموت را منعکس کند. ممکن است تمام داده‌های لازم برای انجام این کار را دانلود کند، اما در واقع هیچ یک از فایل‌های محلی شما را تغییر _نمی‌دهد_. ما در درس‌های بعدی دستوراتی را برای انجام همین کار یاد خواهیم گرفت :D",
              "",
              "بنابراین در پایان روز، می‌توانید اجرای `git fetch` را به عنوان یک مرحله دانلود در نظر بگیرید."
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "برای پایان دادن به مرحله، به سادگی `git fetch` کنید و تمام کامیت‌ها را دانلود کنید!"
            ]
          }
        }
      ]
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "Você perceberá que conforme atualizarmos a representação do repositório remoto, nossas branches _remotas_ irão se atualizar para refletir essa nova representação. Isso tem a ver com o que vimos na lição anterior sobre branches remotas."
            ]
          }
        },
        {
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              ...
              "* Atualiza a referência para a qual as braches remotas (por exemplo, `o/main`) estão apontando",
              "",
              ...
              "",
              "Se você lembrar da lição anterior, nós dissemos que as braches remotos refletem o estado dos repositórios remotos _desde a última vez_ na qual você falou com esses repositórios. O `git fetch` é a única forma de falar com esses repositórios remotos! Espero que a conexão entre as braches remotas e o `git fetch` esteja clara agora.",
              "",
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "O `git fetch`, no entanto, não muda nada do estado _local_ do repositório. Ele não atualiza a sua branch `main` nem muda nada na forma como o seu sistema de arquivos está no momento.",
              "",
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## Git Fetch",
              "",
              "Работата с отдалечени Git хранилища в крайна сметка се свежда до прехвърляне на данни _към_ и _от_ други хранилища. Докато можем да изпращаме комити напред и назад, можем да споделяме всякакъв тип промени, които Git проследява (а с това и работа, нови файлове, нови идеи, любовни писма и т.н.).",
              "",
              "В този урок ще научим как да изтегляме данни _от_ отдалечено хранилище — командата за това съвсем логично се нарича `git fetch`.",
              "",
              "Ще забележиш, че когато обновим нашето представяне на отдалеченото хранилище, и _отдалечените_ бранчове ще се обновят, за да отразят това ново състояние. Това е пряко свързано с предишния урок за remote бранчовете."
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Преди да навлизаме в детайлите на `git fetch`, нека го видим в действие! Тук имаме отдалечено хранилище, което съдържа два комита, които липсват в локалното ни хранилище."
            ],
            "afterMarkdowns": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "### Какво прави fetch",
              "",
              "`git fetch` изпълнява точно две основни стъпки — и само тези две:",
              "",
              "* изтегля комитите, които съществуват в отдалеченото хранилище, но липсват в локалното, и…",
              "* обновява към кои комити сочат отдалечените бранчове (например `o/main`)",
              "",
              "`git fetch` по същество синхронизира _локалното_ ни представяне на отдалеченото хранилище с това как изглежда _реалното_ отдалечено хранилище в момента.",
              "",
              "Ако си спомняш от предишния урок, казахме, че remote бранчовете отразяват състоянието на отдалечените хранилища _от последния път_, когато си „говорил“ с тях. `git fetch` е начинът да осъществиш този контакт! Надявам се връзката между remote бранчовете и `git fetch` вече да е ясна.",
              "",
              "`git fetch` обикновено комуникира с отдалеченото хранилище през интернет (чрез протокол като `http://` или `git://`).",
              ""
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "### Какво fetch НЕ прави",
              "",
              "`git fetch` обаче не променя нищо в _локалното_ ти състояние. Той няма да обнови бранча ти `main`, нито ще промени как изглеждат файловете ти в момента.",
              "",
              "Това е важно да се разбере, защото много разработчици смятат, че изпълнението на `git fetch` автоматично синхронизира локалната им работа с отдалеченото хранилище. Командата може да изтегли всички необходими данни за това, но _не_ променя реално нито един от локалните ти файлове. В следващите уроци ще научим командите, които правят точно това :D",
              "",
              "В крайна сметка можеш да мислиш за `git fetch` просто като за стъпка по изтегляне."
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "За да завършиш нивото, просто изпълни `git fetch` и изтегли всички комити!"
            ]
          }
        }
      ]
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## Git Fetch",
              "",
              "Робота з віддаленими git-репозиторіями зводиться до передачі даних _до_ та _з_ інших репозиторіїв. Можливість передавати коміти дозволяє нам ділитися будь-якою інформацією, що відстежується gitом (а отже, виконаною роботою, новими файлами, ідеями, листами, тощо).",
              "",
              ...
              "",
              "Зауважте, що коли ми оновлюємо наш віддалений репозиторій, наші _віддалені_ гілки теж оновляться. Про це ми говорили на попередньому уроці."
            ]
          }
        },
        {
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              ...
              "* оновлює посилання віддаленого гілці (наприклад, `o/main`)",
              "",
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "Тим не менш, `git fetch` нічого не змінює в _твоєму_ локальному стані. Він не оновить Вашу гілку `main` і не змінить того, як наразі виглядає локальна файлова система.",
              "",
              "Це важливо зрозуміти, тому що багато розробників думають, що `git fetch` оновить їхні локальні дані до стану віддаленого репозиторію. Він дійсно звантажить всі потрібні дані, щоб це зробити, але він _не змінить_ автоматично жодних локальних файлів. Ми вивчимо команди, які це роблять, в наступних уроках :D",
              "",
              "Отже, зрештою, Ви можете вважати що `git fetch` просто звантажує нову інформацію з віддаленого сховища."
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "Щоб пройти цей рівень просто виконайте `git fetch` і звантажте всі коміти!"
            ]
          }
        }
      ]
