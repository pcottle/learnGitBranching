exports.level = {
  ...
  "name": {
    ...
    "bg": "Сливане с отдалечени клонове",
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "Para enviar novas atualizações ao repositório remoto, tudo que você precisa é *incorporar* as últimas mudanças ali presentes. Isso significa que você pode tanto fazer um rebase *quanto* um merge na branch remota (ex. `o/main`).",
              "",
      ...
      childViews: [
        {
          ...
          options: {
            markdowns: [
              "## Защо да не сливане?",
              "",
              "За да пуснеш нови промени към отдалечения репозиториум, всичко, което трябва да направиш, е да *включиш* последните промени от отдалечения клон. Това означава, че можеш да направиш rebase *или* merge към отдалечения клон (например `o/main`).",
              ""
            ]
          }
        },
        {
          ...
          options: {
            markdowns: [
              "Има много дебати сред разработчиците за предимствата и недостатъците на merge срещу rebase. Ето основните плюсове и минуси на rebase:",
              "",
              "Предимства:",
              "",
              "* Rebase прави дървото на комитите много чисто, защото всичко е подредено в една права линия.",
              "",
              "Недостатъци:",
              "",
              "* Rebase променя (видимата) история на дървото на комитите.",
              "",
              "Например, комитът `C1` може да бъде rebase-нат *след* `C3`. Тогава изглежда, че работата по `C1'` е след `C3`, въпреки че всъщност е била завършена преди това.",
              "",
              "Някои разработчици предпочитат да запазят историята и затова избират merge. Други (като мен) предпочитат чисто дърво на комитите и предпочитат rebase. Всичко е въпрос на предпочитания :D"
            ]
          }
        },
        {
          ...
          options: {
            markdowns: [
              "За този ниво, нека се опитаме да решим предишното, но с *сливане* вместо rebase. Може да стане малко объркано, но добре илюстрира идеята."
            ]
          }
        }
      ]
