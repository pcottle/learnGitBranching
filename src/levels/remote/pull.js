          ...
          "childViews": [
              {
                  ...
                  "options": {
                      "markdowns": [
                          ...
                          "* и т.д.",
                          "",
                          "По факту процедура *скачивания (fetching)*  изменений с удалённой ветки и *объединения (merging)* на столько частая и общая что git предоставляет вместо двух команд - одну! Эта команда  - `git pull`."
                      ]
                  }
              },
              {
                  ...
                  "options": {
                      ...
                      "afterMarkdowns": [
                          "Опа -- мы скачали `C3` с помощью команды `fetch` и зтем объединяем эти наработки с помощью `git merge o/master`. Теперь наша ветка `master` отображает изменения с удалённого репозитория (в данном случае с репозитория `origin`)"
                      ],
                      "command": "git fetch; git merge o/master",
                      "beforeCommand": "git clone; git commit; git fakeTeamwork"
                  }
              ...
              },
              {
                  ...
                  "options": {
                      "markdowns": [
                          ...
                          "",
                          "Помните -- мы также можете выполнить этот уровень с помощью комманд `fetch` и `merge`, но нужно ли делать так, когда можно воспользоваться одной коммандой ? :P"
                      ]
                  }
              }
          ]
