exports.level = {
  ...
  "hint": {
    ...
    "bg": "Помни, че трябва да клонираш (clone), преди да можеш да push-ваш!",
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## Git Push",
              "",
              "Добре, значи изтеглих промените от отдалеченото хранилище (remote) и ги включих в моята локална работа. Това е чудесно... но как да споделя _моята_ страхотна работа с всички останали?",
              "",
              "Е, начинът за качване на споделена работа е обратен на изтеглянето ѝ. И какво е обратното на `git pull`? `git push`!",
              "",
              "`git push` отговаря за качването на _твоите_ промени в посочено отдалечено хранилище и обновяването му, така че да включва новите ти къмити. След като `git push` приключи, всички твои приятели ще могат да изтеглят работата ти оттам.",
              "",
              "Можеш да мислиш за `git push` като команда за \"публикуване\" на твоята работа. Тя има доста тънкости, в които ще навлезем скоро, но нека започнем с малки стъпки...",
              "",
              "*бележка -- поведението на `git push` без аргументи варира в зависимост от една git настройка, наречена `push.default`. Стойността по подразбиране зависи от версията на git, която използваш, но в нашите уроци ще използваме стойността `upstream`. Това не е голяма работа, но си струва да провериш настройките си, преди да push-ваш в собствените си проекти.*"
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Тук имаме някои промени, които отдалеченото хранилище няма. Нека ги качим!"
            ],
            "afterMarkdowns": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "За да завършиш това ниво, просто сподели два нови къмита с отдалеченото хранилище. Дръж се здраво обаче, защото тези уроци скоро ще станат много по-трудни!"
            ]
          }
        }
      ]
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "Отже, надсилання даних є, по-суті, протилежним завантаженню даних. А який антонім до `git pull` (притягнути)? `git push` (відштовхнути)!",
              "",
              "`git push` використовується для надсилання _локальних_ змін на вказаний віддалений репозиторій; ця команда оновлює віддалений репозиторій, інтегруючи нові коміти. Після виконання `git push` всі Ваші друзі зможуть завантажити Ваші напрацювання з віддаленого сховища.",
              "",
              "Ви можете вважати, що `git push` \"публікує\" Ваші напрацювання. В цієї команди є кілька особливостей, які ми скоро розглянемо, але почнімо з початку...",
              "",
              "*Зауваження: поведінка `git push` без параметрів різниться в залежності від налаштування git з назвою `push.default`. Значення за замовчуванням цього налаштування залежить від версії Вашого git, але в наших уроках ми будемо вважати що воно дорівнює `upstream`. Це не вкрай важливо, але буде корисно перевірити це налаштування перед тим як пушити свій проект.*"
            ]
          }
        },
        {
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "Щоб пройти цей рівень, просто надішліть два коміти у віддалений репозиторій. Але пристібніться, скоро наші уроки стануть значно важчими!"
            ]
          }
        }
      ]
