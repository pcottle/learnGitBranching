exports.level = {
  ...
  "name": {
    ...
    "fa": "شاخه‌های ریموت (Remote Branches)",
    "zh_CN": "远程分支",
  ...
  "hint": {
    ...
    "fa": "به ترتیب توجه کنید -- ابتدا روی main کامیت کنید!",
    ...
    "bg": "Обърни внимание на реда -- първо направи къмит в main!",
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## شاخه‌های ریموت گیت",
              "",
              "حالا که `git clone` را در عمل دیدید، بیایید بررسی کنیم که واقعاً چه چیزی تغییر کرده است.",
              "",
              "اولین چیزی که ممکن است متوجه شده باشید این است که یک شاخه جدید در مخزن محلی ما به نام `o/main` ظاهر شده است. این نوع شاخه، شاخه _ریموت_ نامیده می‌شود؛ شاخه‌های ریموت ویژگی‌های خاصی دارند زیرا هدف منحصر به فردی را دنبال می‌کنند.",
              "",
              "شاخه‌های ریموت وضعیت مخازن ریموت را منعکس می‌کنند (از آخرین باری که با آن مخازن ریموت ارتباط برقرار کردید). آن‌ها به شما کمک می‌کنند تا تفاوت بین کار محلی خود و آنچه عمومی است را درک کنید -- گامی حیاتی که باید قبل از اشتراک‌گذاری کار خود با دیگران بردارید.",
              "",
              "شاخه‌های ریموت این ویژگی خاص را دارند که وقتی آن‌ها را checkout می‌کنید، در حالت `HEAD` جدا شده قرار می‌گیرید. گیت این کار را عمداً انجام می‌دهد زیرا نمی‌توانید مستقیماً روی این شاخه‌ها کار کنید؛ باید جای دیگری کار کنید و سپس کار خود را با ریموت به اشتراک بگذارید (که پس از آن شاخه‌های ریموت شما به‌روزرسانی می‌شوند).",
              "",
              "برای روشن شدن موضوع: شاخه‌های ریموت در مخزن _محلی_ شما قرار دارند، نه در مخزن ریموت."
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "### `o/` چیست؟",
              "",
              "ممکن است تعجب کنید که `o/` در ابتدای این شاخه‌های ریموت برای چیست. خوب، شاخه‌های ریموت نیز دارای یک قرارداد نام‌گذاری (اجباری) هستند -- آن‌ها با فرمت زیر نمایش داده می‌شوند:",
              "",
              "* `<نام ریموت>/<نام شاخه>`",
              "",
              "بنابراین، اگر به شاخه‌ای به نام `o/main` نگاه کنید، نام شاخه `main` و نام ریموت `o` است.",
              "",
              "اکثر توسعه‌دهندگان در واقع ریموت اصلی خود را `origin` می‌نامند، نه `o`. این کار آنقدر رایج است که وقتی شما یک مخزن را `git clone` می‌کنید، گیت به طور خودکار ریموت شما را `origin` نام‌گذاری می‌کند.",
              "",
              "متأسفانه نام کامل `origin` در رابط کاربری ما جا نمی‌شود، بنابراین ما از `o` به عنوان مخفف استفاده می‌کنیم :( فقط به یاد داشته باشید که وقتی از گیت واقعی استفاده می‌کنید، ریموت شما احتمالاً `origin` نام خواهد داشت!",
              "",
              "این مطالب زیادی برای یادگیری بود، پس بیایید همه این‌ها را در عمل ببینیم."
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "بیایید یک شاخه ریموت را checkout کنیم و ببینیم چه اتفاقی می‌افتد."
            ],
            "afterMarkdowns": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "برای پایان این مرحله، یک بار روی `main` کامیت کنید و یک بار پس از checkout کردن `o/main`. این کار به شما کمک می‌کند تا درک کنید که شاخه‌های ریموت چگونه متفاوت رفتار می‌کنند و فقط برای منعکس کردن وضعیت ریموت به‌روزرسانی می‌شوند."
            ]
          }
        }
      ]
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "Pour être clair : les branches distantes sont sur votre dépôt _local_, pas sur le dépôt distant."
            ]
          }
        },
        {
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "Сега, след като видя `git clone` в действие, нека се потопим в това какво всъщност се промени.",
              "",
              "Първото нещо, което може да си забелязал, е че се появи нов клон в нашето локално хранилище, наречен `o/main`. Този тип клон се нарича _отдалечен_ (remote) клон; отдалечените клонове имат специални свойства, защото служат за уникална цел.",
              "",
              "Отдалечените клонове отразяват _състоянието_ на отдалечените хранилища (от последния път, когато си комуникирал с тях). Те ти помагат да разбереш разликата между твоята локална работа и това, което е публично -- критична стъпка преди да споделиш работата си с другите.",
              "",
              "Отдалечените клонове имат специалното свойство, че когато ги изтеглиш (checkout), попадаш в режим на отделен (detached) `HEAD`. Git прави това нарочно, защото не можеш да работиш директно върху тези клонове; трябва да работиш другаде и след това да споделиш работата си с отдалеченото хранилище (след което твоите отдалечени клонове ще бъдат обновени).",
              "",
              "За да е ясно: Отдалечените клонове са в твоето _локално_ хранилище, а не в отдалеченото."
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              "### Какво е `o/`?",
              "",
              "Може би се чудиш за какво е това `o/` в началото на имената на отдалечените клонове. Е, отдалечените клонове също имат (задължителна) конвенция за наименуване -- те се показват във формат:",
              "",
              "* `<име на отдалечено хранилище>/<име на клон>`",
              "",
              "Следователно, ако гледаш клон с име `o/main`, името на клона е `main`, а името на отдалеченото хранилище е `o`.",
              "",
              "Повечето разработчици всъщност именуват основното си отдалечено хранилище `origin`, а не `o`. Това е толкова често срещано, че git автоматично настройва името на отдалеченото хранилище на `origin`, когато клонираш (`git clone`) дадено хранилище.",
              "",
              "За съжаление пълното име `origin` не се побира в нашия интерфейс, затова използваме `o` като съкращение :( Просто помни, че когато използваш истински git, твоето отдалечено хранилище вероятно ще се казва `origin`!",
              "",
              "Това е много информация за осмисляне, така че нека видим всичко това в действие."
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Нека изтеглим (check out) отдалечен клон и да видим какво ще стане."
            ],
            "afterMarkdowns": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "За да завършиш това ниво, направи един къмит в `main` и веднъж след като изтеглиш (checkout) `o/main`. Това ще ти помогне да разбереш как отдалечените клонове се държат различно и че се обновяват само за да отразят състоянието на отдалеченото хранилище."
            ]
          }
        }
      ]
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              "## Віддалені гілки",
              "",
              "Тепер, коли Ви познайомилися з `git clone` в дії, розгляньмо деталі й подивімося, що дійсно змінилося.",
              "",
              "Перше, що Ви могли помітити -- це те, що з’явилася нова гілка з назвою `o/main`. Такі гілки називаються  _віддаленими_ (remote); віддалені гілки в гіт відіграють в певному сенсі унікальну роль, тому в них є деякі спеціальні властивості, непритаманні іншим гілкам.",
              "",
              "Віддалені гілки відображають _стан_ віддалених репозиторіїв (точніше, стан віддаленого репо на момент останньої синхронізації). Вони дозволяють  відрізняти та відстежувати локальні зміни та зміни інших учасників, що є дуже важливим для успішної синхронізації роботи між різними репозиторіями.",
              "",
              "Важливою властивістю віддалених гілок є те, що коли перейти на них, Ви опинитеся в стані `detached HEAD`. Git робить це спеціально, оскільки неможливо працювати з ними напряму; Ви маєте працювати в локальній гілці й по необхідності синхронізуватися з віддаленим репозиторієм (після чого віддалену гілку буде оновлено)."
            ]
          }
        },
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "Ви, можливо, здогадуєтеся для чого потрібен префікс `o/` на віддалених гілках. Так, існує (примусове) правило іменування віддалених гілок  -- вони відображаються в форматі:",
              "",
              ...
              "",
              "Більшість розробників насправді називають ім’я головного віддаленого репозиторію `origin` (початок), а не `o`. Це настільки поширена практика, що гіт автоматично називає віддалений репозиторій `origin` коли Ви його клонуєте.",
              "",
              "На жаль повністю ім’я `origin` не влазить в наш UI, натомість ми будемо використовувати коротше `o` :( Просто не забудьте, коли будете використовувати звичайний гіт, що Ваш віддалений репо скоріш за все називається `origin`!",
              "",
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "Щоб пройти цей рівень, зробіть один коміт в `main`, а потім перейдіть в `o/main` і закомітся ще раз. Це наочно продемонструє поведінку віддалених гілок, а також покаже як зміни впливають на стан віддаленого репозиторію."
            ]
          }
        }
      ]
