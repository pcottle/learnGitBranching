exports.level = {
  ...
  "hint": {
    ...
    "bg": "Помни, че има два начина за настройване на отдалечено проследяване!",
    "ru_RU": "Помни, есть два способа установить слежку за удалённым репозиторием!",
    "ko": "원격 추적하기를 설정하는데에는 두가지 방법이 있습니다!",
    "uk": "Пам'ятайте, є два способи стежити за віддаленим репозиторієм!",
    ...
    "ru_RU": {
      "childViews": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "Хорошо! Для выполнения этого уровня давайте выполним push наших наработок в ветку main на удалённом репозитории, при этом не скачивая и не создавая ветку main локально. Вместо этого вам следует создать ветку с именем side, которая будет показана на визуализации цели уровня."
            ]
          }
        }
      ]
      ...
      "childViews": [
        {
          ...
          "options": {
            "markdowns": [
              ...
              "",
              "* Під час операції `pull`, коміти потрапляють в `o/main`, а вже потім *мерджаться* в локальний `main`. Гілка в яку відбудеться мердж якраз і визначається цим зв'язком.",
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "### А чи можна обирати самостійно?",
              "",
              "Так, можете! Ви можете вибрати довільну гілку, яка стежитиме за `o/main`, і тоді для цієї гілки `push` та `merge` автоматично працюватимуть з `main`. Це означає, що виконання `git push` в гілці з назвою `totallyNotMain` (зовсім не main) може зберегти локальні коміти у віддалену гілку `main`!",
              "",
              "Є два шляхи встановити такий зв'язок. Перший - створити нову гілку з явним вказанням зв'язку (за ким стежити). Виконання",
              "",
              "`git checkout -b totallyNotMain o/main`",
              "",
              "створить гілку `totallyNotMain`, яка стежить за `o/main`."
            ]
          }
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Досить розмов, подивімось в дії! Створімо нову гілку `foo` і змусьмо її стежити за віддаленою гілкою `main`."
            ],
            "afterMarkdowns": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "### Спосіб №2",
              "",
              "Інший спосіб вказати за якою віддаленою гілкою стежити -- просто використовувати опцію `git branch -u`. Виконання",
              "",
              "`git branch -u o/main foo`",
              "",
              "змусить гілку `foo` стежити за `o/main`. Якщо `foo` є поточною гілкою, її можна навіть не вказувати:",
              "",
              "`git branch -u o/main`",
              ""
            ]
        ...
        },
        {
          ...
          "options": {
            "beforeMarkdowns": [
              "Подивімось на цей спосіб швидко вказати віддалену гілку для відстеження..."
            ],
            "afterMarkdowns": [
        ...
        },
        {
          ...
          "options": {
            "markdowns": [
              "Гаразд! На цьому рівні збережімо Вашу роботу у віддалену гілку `main` *без* переходу на локальну `main`. Про решту здогадайтеся самі, раз Ви вже дійшли до цього рівня :P"
            ]
          }
        }
      ]
